...
export default async function BlogPostPage({
  ...
  // Use publishedAt if available (Sanity), fall back to date (hardcoded)
  const publishDate = (sanityPost?.publishedAt || article.date) as string;
  ...
  // Determine if we have full Sanity content
  const hasFullContent = sanityPost && sanityPost.content && sanityPost.content.length > 0;
      ...
      <main className="min-h-screen pt-28">
        <article className="max-w-4xl mx-auto px-6 lg:px-10 py-16 md:py-24">
          ...
          <div className="reveal">
            {hasFullContent ? (
              <PortableTextRenderer content={sanityPost.content} />
            ) : (
